...
export function Insights() {
  ...
  const [availableLines, setAvailableLines] = useState(['All Lines']);
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5293/api/data/lines')
      ...
      .then(data => {
        ...
        setAvailableLines(['All Lines', ...names]);
      })
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-gray-900">Safety Insights</h2>
          <p className="text-xs text-gray-400 mt-0.5">Violation trends across the network</p>
        </div>
        <select
          value={lineFilter}
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-semibold text-gray-800">By Line (Resolved cases)</span>
        </div>
        ...
        {recentReports.map((r: any) => (
          <div key={r.id} className="flex items-center justify-between py-2.5 border-b border-gray-50 last:border-0">
            ...
            <div className="flex items-center">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${r.status === 'Verified' ? 'bg-green-50 text-green-600' :
