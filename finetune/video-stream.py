...
# ───────────────── S3 ─────────────────
_s3_client      = None
_s3_creds_expiry = 0   # epoch seconds
...
mobilenet = torch.jit.load("/home/iriss-xyii/model_torchscript.pt", map_location="cpu")
mobilenet.eval()
...
def predict_gender(bgr_crop):
    rgb_crop = cv2.cvtColor(bgr_crop, cv2.COLOR_BGR2RGB)  # OpenCV is BGR, model expects RGB
...
last_time          = 0
last_detections    = []   # persists boxes across frames between inference intervals
confirmed_male_ids = set()  # track IDs confirmed as male — skip re-classification and re-upload
...
while True:
    ...
    if now - last_time > INFERENCE_INTERVAL:
        ...
        # persist=True keeps ByteTrack state between calls so IDs follow people as they move
        results = yolo.track(frame, conf=0.5, persist=True, verbose=False)

        # prune confirmed males that are no longer in the frame
        active_ids = {int(box.id[0]) for r in results for box in r.boxes if box.id is not None}
        confirmed_male_ids &= active_ids
        ...
        for r in results:
            for box in r.boxes:
                ...
                # already confirmed male — draw box, skip MobileNet and upload
                if track_id is not None and track_id in confirmed_male_ids:
                    last_detections.append((x1, y1, x2, y2, yolo_conf, "male", 1.0))
                    continue
                ...
                if gender_label == "male" and male_prob > CONF_THRESHOLD:
                    if track_id is not None:
                        confirmed_male_ids.add(track_id)
                    print(f"[ID {track_id}] High-confidence male — uploading crop")
                    url = upload_to_s3(person, male_prob)
                    if url:
